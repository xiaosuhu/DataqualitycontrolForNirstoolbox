        ...
        function TmpDriviate = runThis(obj,data)
                    ...
                    if length(lsthbo)==length(lsthbr)
                        for j=1:numel(obj.lag)
                            for k=1:length(lsthbo)
                                ...
                                TmpDriviate(i,j,k,1)=mean(abs(diff(data(i).data(:,lsthbo(j)),obj.lag(j))))/obj.lag(j);
                                % Temporal Derivatives Check HbR
                                TmpDriviate(i,j,k,2)=mean(abs(diff(data(i).data(:,lsthbr(j)),obj.lag(j))))/obj.lag(j);
                            end
                        end
                    else
            ...
            for j=1:numel(obj.lag)
                ...
                for i=1:size(sum(squeeze(TmpDriviate(:,j,:,1)),2),1)
                    ...
                    h=text(i,max(sum(squeeze(TmpDriviate(:,j,:,1)),2)),name);
                    set(h,'Rotation',60)
                end
                ...
                for i=1:size(sum(squeeze(TmpDriviate(:,j,:,2)),2),1)
                    [~,name,~] = fileparts(data(i).description);
                    h=text(i,max(sum(squeeze(TmpDriviate(:,j,:,2)),2)),name);
                    set(h,'Rotation',60)
                end
            end
        end
